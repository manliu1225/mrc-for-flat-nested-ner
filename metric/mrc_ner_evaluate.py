...
def flat_transform_bmes_label(start_labels, end_labels, span_labels, ner_cate, threshold=0.5):
    ...
    # change positon, first end and then start, if no predicted start, only E will left.
    ...
    for tmp_start in start_labels:
        ...
        if span_labels[tmp_start][tmp_end] >= threshold:
            if tmp_start != tmp_end:
                # new change
                bmes_labels[tmp_start] = "B-{}".format(ner_cate)
                bmes_labels[tmp_end] = "E-{}".format(ner_cate)
...
def flat_ner_performance(pred_start, pred_end, pred_span, \
    ...
    if dims == 1:
        ...
        # print(pred_bmes_label, gold_bmes_label)
        ...
        # print(pred_bmes_idx_lst, up_label_lst)
